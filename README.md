...
manager プロセスは `SakuraSimpleMonitor` リソースを watch し、spec に対応するさくらのクラウドのシンプル監視を作成・更新します。同期後はさくらのクラウド API からヘルス状態を取得し、`status.health` に反映します。削除時の外部リソース削除、finalizer 処理、既存シンプル監視の採用はまだ行いません。
